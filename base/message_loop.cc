...
{
  post_task_internal(queued_task{std::move(action)});
}

void message_loop::post_task(callable action, time_delay delay)
{
  post_task_internal(queued_task{std::move(action), high_steady_clock::now() + delay});
}
...
void message_loop::post_task_internal(queued_task t)
{
//  if (!active_) return;
